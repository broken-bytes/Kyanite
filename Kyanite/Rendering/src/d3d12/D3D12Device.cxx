...
namespace Renderer {
	...
	auto D3D12Device::Name() -> const wchar_t* {
		return L"Default Device";
	}

	auto D3D12Device::VRAM() -> const uint32_t {
		return 0;
	}

	auto D3D12Device::CreateSwapChain(
		...
		return std::make_shared<D3D12SwapChain>(queue, reinterpret_cast<HWND>(_window), *this, frameCount, width, height);
	}

	auto D3D12Device::CreateCommandAllocator(CommandType type)->std::shared_ptr<Allocator> {
		Microsoft::WRL::ComPtr<ID3D12CommandAllocator> alloc;
		_device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE(type), IID_PPV_ARGS(&alloc));
		return std::make_shared<D3D12Allocator>(type, alloc);
	}

	auto D3D12Device::CreateCommandQueue(CommandType type)->std::shared_ptr<CommandQueue> {
	...
	auto D3D12Device::CreateCommandList(
		...
		_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE(type), static_pointer_cast<D3D12Allocator>(allocator)->Native(), nullptr, IID_PPV_ARGS(&commandList));
	...
	auto D3D12Device::CreateRenderTargetHeap(size_t count)->std::shared_ptr<Heap> {
	...
	auto D3D12Device::CreateConstantBufferHeap(size_t count)->std::shared_ptr<Heap> { 
		return std::make_shared<D3D12Heap>(
			*this,
	...
	auto D3D12Device::CreateShaderResourceHeap(size_t count)->std::shared_ptr<Heap> {
		return std::make_shared<D3D12Heap>(
			*this,
			D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV,
			count,
	...
	auto D3D12Device::CreateUnorderedAccessHeap(size_t count)->std::shared_ptr<Heap> { 
		return std::make_shared<D3D12Heap>(
			*this,
			D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV,
			count
	...
	auto D3D12Device::CreateDepthStencilHeap(size_t count)->std::shared_ptr<Heap> { 
		return std::make_shared<D3D12Heap>(
			*this,
	...
	auto D3D12Device::CreateVertexBuffer(std::vector<Vertex> vertices)->std::shared_ptr<Renderer::Buffer> {
		auto len = sizeof(Vertex);
		const uint32_t vertexBufferSize = vertices.size() * sizeof(Vertex);
		auto heapProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
		auto resourceDesc = CD3DX12_RESOURCE_DESC::Buffer(vertexBufferSize, D3D12_RESOURCE_FLAG_NONE);
		...
		return std::make_shared<D3D12Buffer<D3D12_VERTEX_BUFFER_VIEW>>(buffer, view, vertices.size());
	}

	auto D3D12Device::CreateIndexBuffer(std::vector<uint32_t> indices)->std::shared_ptr<Renderer::Buffer> {
		const uint32_t indexBufferSize = indices.size() * sizeof(uint32_t);
		auto heapProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
		auto resourceDesc = CD3DX12_RESOURCE_DESC::Buffer(indexBufferSize, D3D12_RESOURCE_FLAG_NONE);
		...
		return std::make_shared<D3D12Buffer<D3D12_INDEX_BUFFER_VIEW>>(buffer, view, indices.size());
	}

	auto D3D12Device::CreateUploadBuffer(size_t sizeInBytes)->std::shared_ptr<Renderer::UploadBuffer> {
		Microsoft::WRL::ComPtr<ID3D12Resource> buffer;

		auto heapProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
		auto resourceDesc = CD3DX12_RESOURCE_DESC::Buffer(sizeInBytes, D3D12_RESOURCE_FLAG_NONE);
	...
	auto D3D12Device::CreateVertexArrayObject(Renderer::VertexBuffer* vertexBuffer, Renderer::IndexBuffer* indexBuffer)->std::shared_ptr<VertexArrayObject> { return nullptr; }
	auto D3D12Device::CreateCommandQueue()->std::shared_ptr<CommandQueue> { return nullptr; }
	auto D3D12Device::CreateTextureBuffer(
		uint16_t width,
		uint16_t height,
		uint8_t channels,
		uint8_t mipLevels,
	...
	auto D3D12Device::CreateConstantBuffer(
		void* data,
		size_t size,
		const char* name
	...
	auto D3D12Device::CreatePipelineState(
		...
		return std::make_shared<D3D12GraphicsPipelineState>(*this, rootSignature, inputLayout, shaderBinding, 0, topology);
	}
	...
	auto D3D12Device::CreateDepthBuffer(
		...
		return std::make_shared<D3D12Buffer<D3D12_DEPTH_STENCIL_VIEW_DESC>>(buffer, view, 0);
	...
	auto D3D12Device::CreateFence(std::uint64_t fenceValue)->std::shared_ptr<Fence> {
	...
	auto D3D12Device::CreateShaderResourceView(std::shared_ptr<TextureBuffer> buffer, std::shared_ptr<DescriptorHandle> handle) -> void {
	...
	auto D3D12Device::CreateConstantBufferView(std::shared_ptr<Heap> heap, std::shared_ptr<UploadBuffer> buffer, std::shared_ptr<DescriptorHandle> cpuHandle) -> void {
		...
		auto cHandle = static_pointer_cast<DescriptorHandleT<D3D12_CPU_DESCRIPTOR_HANDLE>>(cpuHandle);

		_device->CreateConstantBufferView(&desc, cHandle->Handle());
	}

	auto D3D12Device::CreateMipMaps(std::shared_ptr<TextureBuffer> texture, std::shared_ptr<GraphicsCommandList> commandList, uint16_t width, uint16_t height, uint8_t levels) -> void {
	...
	auto D3D12Device::CompileShader(std::string code)->std::shared_ptr<GraphicsShaderBinding> { 
...

		{
			...
			vsResults->GetOutput(DXC_OUT_OBJECT, IID_PPV_ARGS(&binding->VertexShader), nullptr);
		}

		{
			...
			psResults->GetOutput(DXC_OUT_OBJECT, IID_PPV_ARGS(&binding->PixelShader), nullptr);
		}
	...
	auto D3D12Device::Native() -> ID3D12Device4* {
		return _device.Get();
	}
}